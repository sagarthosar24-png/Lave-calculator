...
# (Lower Reservoir RL and Content)
...
if st.button("Get Final Lake Level", type="primary"):
    ...
    if gate_is_open:
        # Flow rate tiers based on Level Difference (Head)
        if head_diff > 3.0:
            active_rate = 0.17
            ...
            active_rate = 0.08  # Default rate for head < 1.5m
            
        gate_vol = active_rate * hours_open

    # Final Total (Start + Gen In - Gate Out)
    final_content = start_content + gen_vol - gate_vol

    # Find Final Level (With Table Boundaries)
    ...
    if gate_is_open:
        st.info(f"Head Difference: **{head_diff:.3f} m** | Selected Rate: **{active_rate} MCM/hr**")
    ...
    with st.expander("Detailed Calculation Log"):
        ...
        if gate_is_open:
            st.write(f"- Gate Discharge Loss ({active_rate} x {hours_open}hr): **{gate_vol:.3f} MCM**")
